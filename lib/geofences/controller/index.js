...
async function getAllGeofences(req, res) {
  try {
    const userId = req.user.id;
    const result = await service.getAllGeofences(userId);
    if (result?.code === 0) {
      return response.success(result?.data, res);
    } else if (result?.code === 1) {
      return response.failedWithMessage(result?.data, res);
...
async function getAllGeofencesAdmin(req, res) {
  try {
    const result = await service.getAllGeofencesAdmin();
    if (result?.code === 0) {
      return response.success(result?.data, res);
    } else if (result?.code === 1) {
      return response.failedWithMessage(result?.data, res);
...
async function getGeofence(req, res) {
  const ttGeoId = req?.ttGeoId
  try {
    const result = await service.getGeofence(ttGeoId);
